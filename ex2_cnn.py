...
class cnn_model(object):
	...
	def fit(self,X_train,y_train):
		...
		tbCallBack = TensorBoard(log_dir='./logs/mnist_drift/drift/30000_{}'.format(self.name),  
                 histogram_freq=0,  
                 write_graph=True,  
                 write_grads=True, 
                 write_images=True,
		...
		if self.name == 'nor':
			...
			print('--------------cor cnn--------------')
			self.model.compile(optimizer='sgd',loss=Custom_loss(w_loss),metrics=['accuracy'])
			self.model.fit(X_train,y_train,epochs=10,batch_size=128,verbose=True,callbacks=[tbCallBack,drawcallback,corcallback])
...
class cor_callback(Callback):
	"""docstring for corresponding_epoch"""
	def __init__(self):
		super(cor_callback, self).__init__()
		self.pre_grad = []
		self.pre_acc = 0
	...
	def on_batch_end(self,batch,logs={}):
		...
		if batch == 0:
				self.pre_acc = cur_acc
				self.pre_grad.append(get_gradients(self.model))
				return
		if self.epoch % 2 ==0:
			self.pre_grad.append(get_gradients(self.model))
			if self.pre_acc < cur_acc:
				pass
			else:
				self.pre_grad[batch]=0
		else:
			cur_grad = get_gradients(self.model)
			temp_grad = self.pre_grad[batch]
			if temp_grad == 0:
				w_loss=0
			else:
				w_loss = self.weight_loss(cur_grad,temp_grad)
...
class draw_callback(Callback):
	...
	def on_train_end(self,batch,logs={}):
		...
		plt.legend()
		plt.savefig('./test/nor_cnn.png'.format(self.name))
