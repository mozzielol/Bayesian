...
class cnn_model(object):
	...
	def transfer(self,X_train,y_train):
		opcallback = op_pre_callback(X_train,y_train,use_pre=False,info=self.info)
		self.model.compile(optimizer='sgd',loss='categorical_crossentropy',metrics=['accuracy'])
...
class op_pre_callback(Callback):
	...
	def __init__(self,X_train,y_train,use_pre=False,info=None):
	...
	def on_train_begin(self,logs={}):
		...
		if G is None:
			...
			print('G is not None!!~')
			self.pre_g = G
		self.FISHER = self.fisher(get_weight_grad(self.model, X[0 * 128:(0 + 1) * 128],
									  y[0 * 128:(0 + 1) * 128]))
...
#
	...
	def on_train_end(self,logs={}):
		...
		self.cur_g = get_weight_grad(self.model,X[0*128:(0+1)*128],y[0*128:(0+1)*128])
